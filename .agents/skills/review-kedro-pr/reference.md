...
1. **Aliases `_load`/`_save` to `load`/`save`** if the subclass defines `_load` or `_save` (legacy pattern).
1. **Wraps `load` and `save`** with `_load_wrapper`/`_save_wrapper` for logging and error handling. Uses `__loadwrapped__`/`__savewrapped__` flags to avoid double-wrapping.
