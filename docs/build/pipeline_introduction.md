...
To use Kedro's automatic dependency resolution, chain your nodes into a [Pipeline][kedro.pipeline.pipeline.Pipeline] object. A Pipeline is a list of nodes using shared variables. That class can be instantiated using the [Pipeline][kedro.pipeline.pipeline.Pipeline] constructor, based on nodes or other pipelines (in which case all nodes from that pipeline will be used).
