...
This guide shows you how to use parameters in your Kedro project.
...
[Parameters can then be used on their own or fed in as function inputs](parameters_explanation.md#how-parameters-work).
...
[This section describes how to change the merging strategy](how_to_advanced_configuration.md#how-to-change-the-merge-strategy-used-by-omegaconfigloader).
...
!!! note
    To **override parameters and other configurations**, such as catalog entries or file paths, or to specify upfront that certain parameters must be set at runtime, use `$runtime_params` with the `OmegaConfigLoader`. Introduced in Kedro `0.18.14`, this feature allows dynamic overrides of various configuration types using the `--params` CLI option. Use it when you need to switch data sources or adjust runtime settings. [Learn more about `$runtime_params`.](how_to_advanced_configuration.md#how-to-override-configuration-with-runtime-parameters-with-the-omegaconfigloader)
...
For complete details on parameter validation, including how to set up Pydantic models, use field constraints, custom validators, and more, see the dedicated [how-to guide on validating parameters](how_to_validate_parameters.md).
