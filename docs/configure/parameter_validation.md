...
- **Multi-type unions are not validated**: Union type hints with multiple non-None types (e.g. `ModelA | ModelB`) are silently skipped. Only `Optional[Model]` (one model type plus `None`) is unwrapped and validated. Support for multi-type unions may be added in a future release.
