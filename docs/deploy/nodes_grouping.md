...
If you want to group nodes differently from the existing pipeline structure, you can use tags or namespaces instead of creating a new pipeline. The `kedro run --pipelines` command accepts a comma-separated list of pipeline names. While you can switch between pipelines in Kedro Viz, the flowchart view does not support collapsing or expanding them.
...
```bash
  kedro run --pipelines=<your_pipeline_name>
```
...
| **Syntax** | `kedro run --pipelines=<your_pipeline_name>` | `kedro run --tags=<your_tag_name>` | `kedro run --namespaces=< namespace1,namespace2 >` |
