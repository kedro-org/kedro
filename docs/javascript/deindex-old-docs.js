// Explanation from RTD on how this script works:
// https://github.com/readthedocs/addons/pull/474#issuecomment-2615984387

// Idempotency guard - prevent duplicate execution
if (window._kedroDeindexExecuted) {
    console.log("Deindex script already executed, skipping duplicate execution");
} else {
    window._kedroDeindexExecuted = true;
    ...
    function _handleReadTheDocsData(data) {
        ...
        // Check if noindex meta tag already exists to prevent duplicates
        if (document.querySelector("meta[name='robots'][content='noindex, nofollow']")) {
            console.log("Noindex meta tag already exists, skipping");
            return;
        }
