...
To benefit from Kedro's automatic dependency resolution is that you can chain your nodes into a [`Pipeline`][kedro.pipeline.Pipeline], which is a list of nodes that use a shared set of variables. That class can be created using the [`pipeline`][kedro.pipeline.modular_pipeline.pipeline] method, based on nodes or other pipelines (in which case all nodes from that pipeline will be used).
