...
Finally, modify the project catalog so that the `example_iris_data` dataset points to a new DBFS location instead of local. You can use Kedro [configuration environments](../kedro_project_setup/configuration.md#additional-configuration-environments) for this.
...
* Install the latest version of Kedro compatible with version `0.17.6`

```console
%pip install "kedro[spark.SparkDataSet]~=0.17.6"
```
