...
The execution timeline of a Kedro pipeline can be thought of as a sequence of actions performed by various Kedro library components, such as the {py:mod}`datasets <kedro-datasets:kedro_datasets>`, [DataCatalog](/api/kedro.io.DataCatalog), [Pipeline](/api/kedro.pipeline.Pipeline), [Node](/api/kedro.pipeline.node.Node) and [KedroContext](/api/kedro.framework.context.KedroContext).
