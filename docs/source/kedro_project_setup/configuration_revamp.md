...
This section contains detailed information about Kedro project configuration.
...
By default, Kedro uses the `ConfigLoader`, for which the relevant API documentation can be found in [kedro.config.ConfigLoader](/kedro.config.ConfigLoader). In the following sections, you can assume the default `ConfigLoader` is used, unless otherwise specified. 
