...
For more information see ["How to specify parameters at runtime"](../configuration/parameters.md#how-to-specify-parameters-at-runtime).
...
### Logging
`logging.yml` is now independent of Kedro's run environment and used only if `KEDRO_LOGGING_CONFIG` is set to point to it. The [documentation on logging](../logging/index.md) describes in detail how logging works in Kedro and how it can be customised.
