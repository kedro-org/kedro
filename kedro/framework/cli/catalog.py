...
@catalog.command("create-schema")
@env_option
@click.option(
    "--filepath",
    "-f",
    type=click.Path(),
    default=".viz/param-schema.json",
    help="Path to save the JSON schema file. Default: .viz/param-schema.json",
)
@click.option(
    "--overwrite",
    is_flag=True,
    help="Overwrite existing schema file if it exists.",
)
@click.pass_obj
def create_schema(
    metadata: ProjectMetadata, env: str, filepath: str, overwrite: bool
) -> None:
    """
    Generate JSON schema from Pydantic models and dataclasses found in pipeline node functions.

    This command analyzes all pipeline nodes to extract Pydantic model and dataclass type hints,
    then generates a JSON schema that can be used for parameter validation and
    form generation in tools like Kedro-Viz.

    Examples:
        kedro catalog create-schema
        kedro catalog create-schema --filepath my-schema.json
        kedro catalog create-schema --overwrite
    """
    from typing import get_type_hints
    ...
    # Check if Pydantic is available (optional for dataclass support)
    try:
        import pydantic
    ...
    # Extract models from all nodes (both Pydantic and dataclasses)
    models = {}
    ...
    for pipeline_name, pipeline in pipeline_dict.items():
        ...
        for node in nodes:
            ...
            try:
                ...
                # Skip if type hints can't be resolved
                continue

            # Extract models from function type hints (both Pydantic and dataclasses)
            for param_name, type_hint in type_hints.items():
                model_name = _extract_model(type_hint, models, BaseModel)
                if model_name:
                    # Store the model with its name for reference
                    models[model_name] = type_hint

    if not models:
        secho(
            "No Pydantic models or dataclasses found in pipeline node functions.",
            fg="yellow",
        )
        return
    ...
    output_path = Path(filepath)
    if _write_schema_file(output_path, schema, overwrite):
        secho(f"Created JSON schema file: {output_path}", fg="green")
        secho(f"Found {len(models)} model(s):", fg="blue")
        for model_name in models.keys():
            secho(f"  - {model_name}", fg="blue")


def _extract_model(
    type_hint: type, existing_models: dict, BaseModel: type | None
) -> str | None:
    """Extract Pydantic model or dataclass from type hint and add to existing_models.

    Returns:
        str | None: Model name if a Pydantic model or dataclass was found, None otherwise.
    """
    import dataclasses
    import typing as _typing
    from typing import get_args, get_origin
    ...
    if origin is _typing.Union:
        ...
        non_none = [a for a in args if a is not type(None)]
        if non_none:
            return _extract_model(non_none[0], existing_models, BaseModel)
        return None

    # Check if it's a Pydantic model (only if Pydantic is available)
    if (
        BaseModel is not None
        and inspect.isclass(type_hint)
        and issubclass(type_hint, BaseModel)
        and hasattr(type_hint, "model_fields")
    ):
        model_name = type_hint.__name__
        if model_name not in existing_models:
            existing_models[model_name] = type_hint
        return model_name
...
def _generate_json_schema(models: dict[str, type]) -> dict:
    """Generate combined JSON schema from multiple Pydantic models and dataclasses."""
    import dataclasses
    ...
    for model_name, model_class in models.items():
        try:
            ...
            if hasattr(model_class, "model_json_schema"):
                # Get the JSON schema for this Pydantic model
                model_schema = model_class.model_json_schema()
                schemas[model_name] = model_schema

                # Collect definitions if present
                if "$defs" in model_schema:
                    definitions.update(model_schema["$defs"])
                elif "definitions" in model_schema:
                    definitions.update(model_schema["definitions"])

            # Check if it's a dataclass
            elif dataclasses.is_dataclass(model_class):
                # Generate JSON schema for dataclass
                model_schema = _dataclass_to_json_schema(model_class)
                schemas[model_name] = model_schema

        except Exception as e:
            secho(
                f"Warning: Could not generate schema for {model_name}: {e}", fg="yellow"
            )
            continue

    # Create combined schema
    if len(schemas) == 1:
        # Single model - return its schema directly
        schema = list(schemas.values())[0]
    else:
        # Multiple models - create a combined schema
        schema = {
            "type": "object",
            "title": "Kedro Parameters Schema",
            "description": "Combined schema for all models found in pipeline nodes",
            "properties": {},
            "anyOf": [],
        }

        for model_name, model_schema in schemas.items():
            # Add each model as a possible schema
            schema["anyOf"].append({"title": model_name, "allOf": [model_schema]})

            # Also add model properties to the top level for easier access
            if "properties" in model_schema:
                schema["properties"].update(model_schema["properties"])
...
def _dataclass_to_json_schema(dataclass_type: type) -> dict:
    ...
    # Remove empty required array for cleaner schema
...
def _type_to_json_schema(type_hint: type) -> dict:
    ...
    if origin is _typing.Union:
        ...
        if len(args) == 2 and type(None) in args:
            # This is Optional[T]
            base_schema = _type_to_json_schema(non_none_types[0])
    ...
    if inspect.isclass(type_hint) and dataclasses.is_dataclass(type_hint):
        return _dataclass_to_json_schema(type_hint)
    ...
    try:
        ...
        if (
            ...
            and hasattr(type_hint, "model_json_schema")
        ):
    ...
    return {"type": "object", "description": f"Unknown type: {type_hint}"}


def _write_schema_file(filepath: Path, schema: dict, overwrite: bool) -> bool:
    ...
    # Write JSON schema file with nice formatting
    with open(filepath, "w") as f:
