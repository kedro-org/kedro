...
class AbstractRunner(ABC):
    ...
    def run(
        ...
        Returns:
            Any node outputs that cannot be processed by the ``CatalogProtocol``.
        ...
        if unsatisfied:
            raise ValueError(
                f"Pipeline input(s) {unsatisfied} not found in the CatalogProtocol"
            )
    ...
    def run_only_missing(
        ...
        Returns:
            ...
            ``CatalogProtocol``. These are returned in a dictionary, where
    ...
    def _suggest_resume_scenario(
        ...
        Args:
            ...
            catalog: the ``CatalogProtocol`` of the run.
...
def run_node(
    ...
    Args:
        ...
        catalog: A ``CatalogProtocol`` containing the node's inputs and outputs.
        hook_manager: The ``PluginManager`` to activate hooks.
        is_async: If True, the node inputs and outputs are loaded and saved
            asynchronously with threads. Defaults to False.
