...
class AbstractRunner(ABC):
    ...
    def run(
        ...
        self._run(pipeline, catalog, hook_manager)
...
def run_node(
    ...
    """
    if is_async:
        node = _run_node_async(node, catalog, hook_manager)
    else:
        node = _run_node_sequential(node, catalog, hook_manager)
...
def _run_node_sequential(
    node: Node, catalog: DataCatalog, hook_manager: PluginManager
) -> Node:
    inputs = {}
    ...
    additional_inputs = _collect_inputs_from_hook(
        node, catalog, inputs, is_async, hook_manager
    )
    inputs.update(additional_inputs)

    outputs = _call_node_run(node, catalog, inputs, is_async, hook_manager)
