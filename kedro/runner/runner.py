...
from kedro.io import AbstractDataCatalog, MemoryDataset
...
class AbstractRunner(ABC):
    ...
    def run(
        ...
        Returns:
            Any node outputs that cannot be processed by the ``AbstractDataCatalog``.
        ...
        if unsatisfied:
            raise ValueError(
                f"Pipeline input(s) {unsatisfied} not found in the AbstractDataCatalog"
            )
    ...
    def run_only_missing(
        ...
        Returns:
            ...
            ``AbstractDataCatalog``. These are returned in a dictionary, where
    ...
    def _suggest_resume_scenario(
        ...
        Args:
            ...
            catalog: the ``AbstractDataCatalog`` of the run.
...
def _find_nodes_to_resume_from(
    pipeline: Pipeline, unfinished_nodes: Collection[Node], catalog: AbstractDataCatalog
) -> set[str]:
...
def _find_all_nodes_for_resumed_pipeline(
    pipeline: Pipeline, unfinished_nodes: Iterable[Node], catalog: AbstractDataCatalog
) -> set[Node]:
...
def _enumerate_non_persistent_inputs(
    node: Node, catalog: AbstractDataCatalog
) -> set[str]:
...
def run_node(
    ...
    Args:
        ...
        catalog: A ``AbstractDataCatalog`` containing the node's inputs and outputs.
        hook_manager: The ``PluginManager`` to activate hooks.
        is_async: If True, the node inputs and outputs are loaded and saved
            asynchronously with threads. Defaults to False.
