...
2. **Schema-on-dataset** — *instead of type hints on nodes* — one entry one validator kills last-wins; catalog is the discoverable, grep-able source of truth (Adeikalam's production pattern, now native). Note for Merel's thread question: the `schemas/` folder is **convention, not enforcement** — any importable dotted path works; no auto-detection because the explicit path IS the discovery.
