...
class TestMessageDataSet:
    ...
    def test_load_missing_file(self, message_data_set):
        ...
        pattern = r"Failed while loading data from data set MessageDataSet\(.*\)"
    ...
    def test_protocol_usage(self, filepath, instance_type):
        data_set = MessageDataSet(filepath=filepath)
...
class TestMessageDataSetVersioned:
    def test_version_str_repr(self, load_version, save_version):
        """Test that version is in string representation of the class instance
        when applicable."""
        filepath = "test"
        ds = MessageDataSet(filepath=filepath)
        ds_versioned = MessageDataSet(
        ...
        assert "MessageDataSet" in str(ds_versioned)
        assert "MessageDataSet" in str(ds)
    ...
    def test_no_versions(self, versioned_message_data_set):
        ...
        pattern = r"Did not find any versions for MessageDataSet\(.+\)"
        with pytest.raises(DataSetError, match=pattern):
    ...
    def test_prevent_overwrite(self, versioned_message_data_set, dummy_msg):
        ...
        versioned_message_data_set.save(dummy_msg)
        pattern = (
            r"Save path \`.+\` for MessageDataSet\(.+\) must "
