...
def call_micropkg_package(
    ...
    result = CliRunner().invoke(
        cli, ["micropkg", "package", pipeline_name, *options], obj=metadata
    )
...
class TestMicropkgPullCommand:
    ...
    def test_pull_local_whl(
        ...
        result = CliRunner().invoke(
            fake_project_cli,
            ["micropkg", "pull", str(wheel_file), *options],
            obj=fake_metadata,
        )
        assert result.exit_code == 0, result.output
    ...
    def test_pipeline_alias_refactors_imports(
        ...
        CliRunner().invoke(
            fake_project_cli,
            ["micropkg", "pull", str(wheel_file), "--alias", pull_alias],
            obj=fake_metadata,
        )
    ...
    def test_micropkg_pull_as_aliased_pipeline_conflicting_name(
        ...
        result = CliRunner().invoke(
            fake_project_cli,
            ["micropkg", "pull", str(wheel_file), "--alias", package_name],
            obj=fake_metadata,
        )
        assert result.exit_code == 0, result.output
        path = fake_package_path / "pipelines" / package_name / "pipeline.py"
    ...
    def test_pull_whl_fs_args(
        ...
        CliRunner().invoke(
            fake_project_cli, ["micropkg", "pull", str(wheel_file), *options]
        )
    ...
    def test_pull_from_pypi(
        ...
        result = CliRunner().invoke(
            fake_project_cli,
            ["micropkg", "pull", PIPELINE_NAME, *options],
            obj=fake_metadata,
        )
        assert result.exit_code == 0
        assert "pulled and unpacked" in result.output
