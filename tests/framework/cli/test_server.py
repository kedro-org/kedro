...
class TestServerStartCommand:
	...
	def test_falls_back_to_discovered_project(self, mocker, tmp_path):
		...
		mock_bootstrap_project = mocker.patch("kedro.framework.cli.server.bootstrap_project")
	...
	def test_happy_path_with_default_options(self, fake_project_cli, fake_metadata, mocker):
		mock_environ = mocker.patch("os.environ", {})
		uvicorn_run = mocker.Mock()
		mocker.patch.dict("sys.modules", {"uvicorn": types.SimpleNamespace(run=uvicorn_run)})

		result = CliRunner().invoke(fake_project_cli, ["server", "start"], obj=fake_metadata)
	...
	def test_happy_path_with_custom_options(self, fake_project_cli, fake_metadata, mocker):
		mock_environ = mocker.patch("os.environ", {})
		uvicorn_run = mocker.Mock()
		mocker.patch.dict("sys.modules", {"uvicorn": types.SimpleNamespace(run=uvicorn_run)})
		...
		result = CliRunner().invoke(
			fake_project_cli,
			["server", "start", "--host", "0.0.0.0", "--port", "9000", "--reload", "--debug"],
			obj=fake_metadata,
		)
	...
	def test_raises_helpful_error_when_uvicorn_is_missing(
		...
		assert isinstance(result.exception, KedroCliError)
		assert "requires 'uvicorn' and 'fastapi' packages" in str(result.exception)
