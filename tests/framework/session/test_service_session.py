...
class TestKedroServiceSession:
    ...
    def test_get_config_loader_trusted_runtime_params_by_default(self, fake_project):
        """`trusted_runtime_params` defaults to True, so the config loader is
        not restricted -- this is the behavior for trusted callers such as
        programmatic use of `KedroServiceSession`."""
        session = KedroServiceSession.create(project_path=fake_project)
        config_loader = session._get_config_loader()
    ...
    def test_get_config_loader_untrusted_runtime_params(self, fake_project):
        """Passing `trusted_runtime_params=False` -- what `http_server.py` does
        for every `POST /run` request -- forces the config loader to restrict
        `runtime_params`-driven catalog `type` selection, regardless of
        `CONFIG_LOADER_ARGS`. See kedro-org/kedro#5706."""
        session = KedroServiceSession.create(project_path=fake_project)
        config_loader = session._get_config_loader(trusted_runtime_params=False)
    ...
    def test_get_config_loader_untrusted_overrides_config_loader_args(
        ...
        """`trusted_runtime_params=False` must force the restriction on even if
        a project's own `CONFIG_LOADER_ARGS` doesn't set it -- an untrusted
        caller's classification must not be weakenable by project config."""
        session = KedroServiceSession.create(project_path=fake_project)
        config_loader = session._get_config_loader(trusted_runtime_params=False)
        assert config_loader.restrict_runtime_params_type_selection is True
