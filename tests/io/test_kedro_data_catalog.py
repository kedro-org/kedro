...
class TestDataCatalog:
    ...
    def test_confirm(self, mocker, caplog):
        ...
        with caplog.at_level(logging.INFO):
            mock_ds = mocker.Mock()
            kedro_data_catalog = DataCatalog(datasets={"mocked": mock_ds})
    ...
    class TestDataCatalogFromConfig:
        ...
        def test_config_invalid_dataset(self, correct_config):
            ...
            pattern = (
                "An exception occurred when parsing config for dataset 'boats':\n"
                "Dataset type 'kedro.io.kedro_data_catalog.DataCatalog' is invalid: "
        ...
        #             }
        #         }
        #         kedro_data_catalog = DataCatalog.from_config(catalog=catalog)
        ...
        def test_bad_confirm(self, correct_config, dataset_name, pattern):
            ...
            does not have `confirm` method"""
            kedro_data_catalog = DataCatalog.from_config(**correct_config)
            with pytest.raises(DatasetError, match=re.escape(pattern)):
                kedro_data_catalog.confirm(dataset_name)
