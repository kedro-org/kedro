...
def test_get_project_path_from_argument(tmp_path):
    assert _resolve_project_path(tmp_path) == Path(tmp_path).resolve()


def test_get_project_path_argument_takes_precedence_over_env_var(
...
def test_get_project_path_raises_when_argument_path_does_not_exist():
    with pytest.raises(ServerSettingsError, match="does not exist"):
