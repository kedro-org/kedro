"""Integration tests for the DataCatalog validation funnel.
...
class TestGenericExceptionWrapping:
    def test_error_severity_wraps_plain_exceptions(self, tmp_path):
        import pandas as pd

        filepath = tmp_path / "cars.csv"
        pd.DataFrame({"a": [1]}).to_csv(filepath, index=False)
        catalog = DataCatalog.from_config(
            {
                "cars": {
                    "type": "pandas.CSVDataset",
                    "filepath": str(filepath),
                    "validator": f"{_MODULE}._raise_value_error",
                }
            }
        )
